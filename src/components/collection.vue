              ...
              <el-row>
                ...
                <el-col :span="4">
                  <el-popconfirm
                    ...
                    icon-color="#626AEF"
                    title="您确定要取消收藏该帖子吗？"
                    @confirm="cancelCollection(slotProp.item.uid)"
                    >
                  <template #reference>
  ...
  export default{
    ...
    methods:{
      async cancelCollection(id){
        try{
          const data={"user_id":this.user_id,"post_id":id};
          const response=await axios.post('http://1.94.170.22:5000/toggle_collection', data);
      ...
      async Getposts(){
        try{
          const response=await axios.get('http://1.94.170.22:5000/get_user_collections?user_id='+this.user_id);
