            ...
            <el-row>
                ...
                <el-col :span="4">
                  <el-popconfirm
                    ...
                    icon-color="#626AEF"
                    title="您确定要取消点赞该帖子吗？"
                    @confirm="cancelLike(slotProp.item.uid)"
                    >
                  <template #reference>
...
export default{
  ...
  methods:{
    ...
    async Getposts(){
      try{
        const response=await axios.get('http://1.94.170.22:5000/get_user_likes?user_id='+this.user_id);
    ...
    async cancelLike(id){
      try{
          ...
          const response=await axios.post('http://1.94.170.22:5000/toggle_like', data);
